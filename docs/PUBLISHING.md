...
> `gh`), see [RELEASING.md](RELEASING.md). The CI release workflow is currently
> unusable — do not release by pushing a tag alone.
...
1. Increment `expo.android.versionCode` in `app.json` (see
   [RELEASING.md](RELEASING.md#versioning) — do **not** edit
