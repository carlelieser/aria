...
export function ResponsiveListItem(props: { data: MusicResponsiveListItem }) {
	...
	if (props.data.item_type === "song") {
		return <TouchableOpacity className={"flex flex-row items-center w-full gap-4 py-4"}
		                         onPress={() => {
			...
			<View className={"flex flex-col gap-1 flex-1"}>
				...
				<Text
					variant={"muted"}>{props.data.artists?.map(artist => artist.name).join(", ")} · {props.data.album?.name}</Text>
			</View>
		</TouchableOpacity>
	}
