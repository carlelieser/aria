...
export class SpotifyAuthManager {
  ...
  async exchangeCode(code: string): Promise<Result<void, Error>> {
    try {
      ...
      if (!response.ok) {
        ...
        return err(new Error(`Token exchange failed: ${error.error_description || error.error}`));
      }
  ...
  async refreshAccessToken(): Promise<Result<void, Error>> {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        return err(new Error(`Token refresh failed: ${error.error_description || error.error}`));
      }
  ...
  /**
   * Internal: Set tokens from response and persist
   */
  ...
  /**
   * Internal: Load tokens from storage
   */
