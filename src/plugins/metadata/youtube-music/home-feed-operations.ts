	...
	return {
		async getHomeFeed(): Promise<Result<HomeFeedData, Error>> {
			try {
				...
				return ok(mapHomeFeedResponse(currentFeed));
			} catch (error) {
				originalFeed = null;
				currentFeed = null;
				return err(
		...
		async getPlaylistTracks(playlistId: string): Promise<Result<Track[], Error>> {
			try {
				...
				logger.info(`Fetched ${tracks.length} tracks from playlist ${playlistId}`);
				return ok(tracks);
			} catch (error) {
				return err(
					error instanceof Error
