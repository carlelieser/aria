/**
 * YouTube Music search operations
 */
...
/**
 * Extract items from search results shelves
 */
...
/**
 * Create search operations with client manager
 */
export function createSearchOperations(clientManager: ClientManager): SearchOperations {
  ...
  return {
    searchTracks(query: string, options?: SearchOptions) {
      return search(query, 'song', (item) => mapYouTubeTrack(item as YouTubeMusicItem), options, 'YouTube Music search failed');
    },

    searchAlbums(query: string, options?: SearchOptions) {
      return search(query, 'album', (item) => mapYouTubeAlbum(item as YouTubeMusicItem), options, 'YouTube Music album search failed');
    },

    searchArtists(query: string, options?: SearchOptions) {
      return search(query, 'artist', (item) => mapYouTubeArtist(item as YouTubeMusicItem), options, 'YouTube Music artist search failed');
    },
  };
}
