...
import { useSearchStore, type SearchSuggestion, type SearchResults as AppSearchResults } from '../state/search-store';
...
/**
 * Search service aggregates results from all enabled metadata providers.
 */
export class SearchService {
  ...
  removeMetadataProvider(providerId: string): void {
    this.metadataProviders = this.metadataProviders.filter(
    ...
    try {
      const searchPromises = this.metadataProviders.map(async (provider) => {
        try {
          ...
          logger.warn(`Search failed for provider ${provider.manifest.id}`, error instanceof Error ? error : undefined);
  ...
  async getSuggestions(query: string): Promise<Result<SearchSuggestion[], Error>> {
    ...
    if (!query.trim()) {
      const recentSuggestions: SearchSuggestion[] = store.recentSearches.map(
        (q) => ({ query: q, type: 'recent' })
      );
      store.setSuggestions(recentSuggestions);
      return ok(recentSuggestions);
    }

    // TODO(#123): Implement suggestion API in metadata providers
