...
interface PlayerState {
  ...
  // Internal state updates (called by playback service)
...
/**
 * Player store for managing playback state and queue
 */
export const usePlayerStore = create<PlayerState>((set, get) => ({
  ...
  skipToNext: () => {
    ...
    const nextIndex = getNextTrackIndex(
      { tracks: state.queue, currentIndex: state.queueIndex, originalOrder: state.originalQueue },
      state.repeatMode
    );
  ...
  setQueue: (tracks: Track[], startIndex = 0) => {
    const state = get();
    const effectiveQueue = state.isShuffled
  ...
  toggleShuffle: () => {
    ...
    if (newShuffleState) {
      // Shuffling: shuffle the original queue, keeping current track at current index
      ...
      // Un-shuffling: restore original queue
      const currentTrack = state.currentTrack;
...
export const useVolume = () => usePlayerStore((state) => ({
  volume: state.volume,
