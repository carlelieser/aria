...
> For cutting a GitHub release (building the APK/IPA locally and publishing with
> `gh`), see [docs/RELEASING.md](docs/RELEASING.md). The CI release workflow is
> currently unusable — do not release by pushing a tag alone.
...
1. Increment `expo.android.versionCode` in `app.json` (see
   [docs/RELEASING.md](docs/RELEASING.md#versioning) — do **not** edit
   `android/app/build.gradle`, which is regenerated by `expo prebuild`)
